...
def PID(reference, measured, total, previous_error, delta):
	...
	immediately preceding step, and the timestep, assumed constant
	'''
...
#def main(argv=None, freq=1000, exptime=2, drivers=None):
	...
	# Load reference trajectory for PID control.
	reference = scipy.io.loadmat('ref_traj.mat')
	reference_trajectory = reference['reference_trajectory']
	...
	for loop_ct in range(nloop):
		...
		drivers["mfb1"].realvalue['da'][2], previous_error, total_error = PID(reference_trajectory[loop_ct,0], results['raw_counter1'][loop_ct,0], total_error, previous_error, 1.0/freq)
